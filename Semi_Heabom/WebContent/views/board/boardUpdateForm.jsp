...
<%@page import="com.heabom.common.model.vo.HashTag"%>
...
<%
	...
	String htagList = (String)(request.getAttribute("htagList"));
            ...
            <div class="write-post-area" style="padding: 10px;">
                <div id="write-post-header">
                    <div id="write-post-title-area">
                        <input type="text" id="write-post-title" name="title" placeholder="제목을 입력하세요." value="<%= b.getBoardTitle() %>" required>
                    </div>
                            ...
                            <script>
                            	$(function(){
                            		$("#select-category option").each(function(){
                            			if($(this).val() == "<%= b.getBoardNo().substring(0,1) %>"){
                    ...
                    <textarea name="content" id="write-post" style="width: 100%; height: 500px; resize: none;" required><%= b.getBoardContent() %></textarea>
                ...
                <script>
                	$(function(){
                		let htagString = ("<%= htagList %>");
                		console.log(htagString+"ㅋㅋ")
                		if(htagString != ['null']){
		        	   		console.log(htagString)
	                		let htagList = (htagString).split(",");
		           //     	for(let index in list){ for in 문은 디버깅용으로 주로 사용
		        	   		console.log(htagList)
		        	   		for(let index of htagList){
								$("#input-tag").prepend('<span class="parent-span">'+
														'<span>#</span><div class="tag-input-area">'+
														'<input class="tag-input" type="text" maxlength="50" placeholder="태그입력" value="'+
														index+'"><button type="button" class="delete-tag">x</button></div></span>')         		
		        	   		}
